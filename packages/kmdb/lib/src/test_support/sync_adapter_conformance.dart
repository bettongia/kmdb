...
import 'gated_sync_adapter.dart';
...
///
/// [expectsCancellation] opts in to the cancellation conformance tests.
/// When `true`, the suite asserts:
/// - A pre-cancelled [CancellationToken] in a [SyncContext] causes each
///   adapter method to throw [SyncCancelledException] at entry (before any
///   I/O is attempted).
/// - An in-flight call blocked on a [GatedSyncAdapter] barrier throws
///   [SyncCancelledException] promptly when the token is cancelled mid-wait.
///
/// Adapters with no long-running waits (e.g. [MemorySyncAdapter]) may pass
/// `expectsCancellation: false` (the default) — they are not required to
/// honour the context.
///
/// Example:
/// ```dart
...
void runSyncAdapterConformance({
  ...
  bool expectsCancellation = false,
}) {
  ...
  if (expectsCancellation) {
    _runCancellationConformanceTests(factory: factory);
  }
}
...
/// Runs the opt-in cancellation conformance tests.
///
/// Tests two properties:
///
/// 1. **Entry cancellation**: each method throws [SyncCancelledException]
///    promptly when called with a pre-cancelled [CancellationToken]. The
///    underlying operation must not complete (verified via the gate / a spy).
///
/// 2. **Mid-flight cancellation**: a call that is suspended on a
///    [GatedSyncAdapter] barrier wakes and throws [SyncCancelledException]
///    when the token is cancelled while the call is in-flight — without the
///    underlying delegate completing the operation.
///
/// This group is called only when `expectsCancellation: true` is passed to
/// [runSyncAdapterConformance].
void _runCancellationConformanceTests({
  required SyncStorageAdapter Function() factory,
}) {
  group('cancellation (conformance)', () {
    // ── Entry cancellation: pre-cancelled token ───────────────────────────────

    test(
      'list throws SyncCancelledException with pre-cancelled token',
      () async {
        final token = CancellationToken()..cancel();
        final ctx = SyncContext(cancel: token);
        final adapter = factory();
        await expectLater(
          adapter.list('dir', ctx: ctx),
    ...
    test(
      'download throws SyncCancelledException with pre-cancelled token',
      () async {
        final token = CancellationToken()..cancel();
        final ctx = SyncContext(cancel: token);
        final adapter = factory();
        await expectLater(
          adapter.download('path/file.bin', ctx: ctx),
          throwsA(isA<SyncCancelledException>()),
        );
      },
    );
    ...
    test(
      'upload throws SyncCancelledException with pre-cancelled token',
      () async {
        final token = CancellationToken()..cancel();
        final ctx = SyncContext(cancel: token);
        final adapter = factory();
        await expectLater(
          adapter.upload('path/file.bin', Uint8List.fromList([1]), ctx: ctx),
          throwsA(isA<SyncCancelledException>()),
        );
      },
    );
    ...
    test(
      'delete throws SyncCancelledException with pre-cancelled token',
      () async {
        final token = CancellationToken()..cancel();
        final ctx = SyncContext(cancel: token);
        final adapter = factory();
        await expectLater(
          adapter.delete('path/file.bin', ctx: ctx),
          throwsA(isA<SyncCancelledException>()),
        );
      },
    );
    ...
    test(
      'compareAndSwap throws SyncCancelledException with pre-cancelled token',
      () async {
        final token = CancellationToken()..cancel();
        final ctx = SyncContext(cancel: token);
        final adapter = factory();
    ...
    test(
      'getEtag throws SyncCancelledException with pre-cancelled token',
      () async {
        final token = CancellationToken()..cancel();
        final ctx = SyncContext(cancel: token);
        final adapter = factory();
        await expectLater(
          adapter.getEtag('path/file.bin', ctx: ctx),
          throwsA(isA<SyncCancelledException>()),
        );
      },
    );
    ...
    test(
      ...
      () async {
        ...
        final future = gated.compareAndSwap(
          'path/file.bin',
          Uint8List.fromList([1]),
          ctx: ctx,
        );
