...
/// should be replaced with (or fall back to) `IcuTokeniser` from the
/// `kmdb_tokenizer_icu` package, which uses the system ICU library and conforms
/// to UAX #29 Unicode Text Segmentation. The [Tokeniser] interface makes that
/// swap transparent to the indexing pipeline.
///
/// ## Why not ICU now?
///
/// Spike investigation confirmed that `IcuTokeniser` works correctly on all
...
/// print(tokeniser.tokenise('Hello, world!')); // ['Hello', 'world']
/// print(tokeniser.tokenise('')); // []
/// print(tokeniser.tokenise('mTLS handshake')); // ['mTLS', 'handshake']
/// ```
class RegExpTokeniser implements Tokeniser {
  /// Creates a new [RegExpTokeniser].
  const RegExpTokeniser();
