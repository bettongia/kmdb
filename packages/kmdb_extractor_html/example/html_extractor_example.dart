...
/// Example: indexing and searching an HTML file's content via KMDB vault
/// search.
///
/// Opens a native, on-disk [KmdbDatabase] with vault search enabled and
/// [HtmlTextExtractor] registered for `text/html` blobs, ingests an HTML
/// file supplied as a command-line argument, waits for indexing to
/// complete, and runs a [KmdbCollection.searchVault] query against it.
///
/// ## Usage
///
/// ```bash
/// dart run example/html_extractor_example.dart /path/to/page.html "some query"
/// ```
///
...
Future<void> main(List<String> args) async {
  final htmlPath = args.isNotEmpty ? args[0] : 'test/fixtures/golden_path.html';
  final query = args.length > 1 ? args[1] : 'quoted thought';

  final htmlFile = File(htmlPath);
  if (!htmlFile.existsSync()) {
    stderr.writeln('HTML file not found: $htmlPath');
    exit(1);
  }

  // Use a fresh temporary directory for this example's database.
  final dbDir = Directory.systemTemp.createTempSync('kmdb_extractor_html_');
  ...
  try {
    ...
    final db = await KmdbDatabase.open(
      ...
      // HtmlTextExtractor is the only extractor most applications need to
      // add for HTML content — PlainTextExtractor (text/plain) is always
      // included automatically.
      vaultSearch: VaultSearchConfig(extractors: [HtmlTextExtractor()]),
    );

    try {
      print('Ingesting $htmlPath into the vault ...');
      ...
      await vaultStore.ingest(
        ...
        hlcTimestamp: DateTime.now().millisecondsSinceEpoch
            .toRadixString(16)
            .padLeft(16, '0'),
        originalName: htmlFile.uri.pathSegments.last,
      ...
      //
      //   final collection = db.rawCollection('pages');
      //   await collection.insert({'title': '...', 'html': ref.uri});
      //
