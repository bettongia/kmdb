...
/// Integration test: registers [HtmlTextExtractor] in a real
/// [VaultSearchConfig] on a real [KmdbDatabase] and ingests an HTML vault
/// blob, confirming that indexing reaches the expected terminal status
/// ([KmdbDatabase.vaultIndexingStatus]).
///
/// This exercises [HtmlTextExtractor] running inside the real, spawned vault
/// indexing isolate (rather than being called directly, as in
/// `html_text_extractor_test.dart`) — reaching `indexed` status is
...
/// `kmdb_extractor_pdf/test/pdf_text_extractor_integration_test.dart`,
/// which documents two *separate* accommodations for pre-existing gaps:
///
...
///
/// Neither gap is fixed here; both are simply worked around the same way
/// WI-8's PDF extractor test was.
///
/// ## Why there is no "corrupt input → failed" scenario here
///
/// Unlike a PDF (which can be password-protected or structurally invalid,
/// causing [PdfTextExtractor] to return `null`), [HtmlTextExtractor] almost
/// never fails: `decodeText()` always succeeds (its fallback charset accepts
/// any byte sequence), and the `html` package's parser is deliberately
/// browser-tolerant — it does not throw on malformed markup, it recovers.
/// So arbitrary/garbage bytes still reach `indexed` status (possibly with
/// sparse or empty extracted text), not `failed`. This is a real, documented
/// behavioral difference from `PdfTextExtractor`, not an oversight in this
/// test's coverage.
library;
...
var _counter = 0;
String _uniquePath() => '/html_extractor_integration_${_counter++}';

/// Reads a fixture HTML file's bytes.
...
void main() {
  ...
  group('HtmlTextExtractor — vault indexing isolate integration', () {
    test('a real, prose-bearing HTML vault blob is extracted and indexed via '
        'the real vault indexing isolate', () async {
      ...
      await vault.ingest(
        ...
        originalName: 'golden_path.html',
        explicitMediaType: 'text/html',
      );
      ...
      expect(
        ...
        equals(1),
        reason:
            'the single ingested HTML blob should have been indexed by '
            'the real vault indexing isolate: $status',
      );
      expect(status.failed, equals(0));
      expect(status.unsupported, equals(0));
    });

    test('an HTML blob that is 100% <script>/<style> is indexed with zero '
        'chunks, not marked failed or unsupported', () async {
      final path = _uniquePath();
      final mem = MemoryStorageAdapter();
      final vault = _TestVaultStore(mem, path);
      ...
      await vault.ingest(
        ...
        originalName: 'all_script_style.html',
        explicitMediaType: 'text/html',
      );
  ...
  group('HtmlTextExtractor — WI-10 encryption integration', () {
    test(
      'an HTML vault blob stored with encryption enabled is decrypted '
      'before HtmlTextExtractor sees it, and indexing still succeeds',
      () async {
        final path = _uniquePath();
        final mem = MemoryStorageAdapter();
        final vault = _TestVaultStore(mem, path);
        ...
        final setup = await EncryptionConfig.createResult(
          passphrase: 'wi9-html-integration-test-passphrase',
        );
