...
**PR link**: https://github.com/bettongia/kmdb/pull/24 (merged)
...
### Step 8 — PR
- [x] Open PR1 against `main`; update **PR link** above; on merge move this
      plan to `docs/plans/completed/` and unblock PR2. (PR #24 merged.)

## Summary

- Added a streaming reclamation transform to
  `packages/kmdb/lib/src/engine/compaction/compaction_job.dart` that groups
  merged entries by `(namespace, userKey)` and keeps only the highest-HLC
  entry per group. Safe at any compaction level (single-file collapse,
  L0→L1, L1→L2) under read-side LWW.
- Introduced `ReclamationPolicy` and `ReclamationPolicyRegistry` in
  `packages/kmdb/lib/src/engine/compaction/reclamation_policy.dart`. The
  default registry collapses every namespace except a configurable
  retain-all prefix list, which defaults to `$ver:`. `CompactionJob` accepts
  an optional registry, defaulting to the standard one — no `LsmEngine` or
...
- Tests added: 9 unit tests for the registry resolver, 6 `CompactionJob`
  ...
  `LsmEngine` test that drives 20 overwrites of one key through
  `compactAll` and asserts exactly one entry survives on disk.
- Specs updated: `docs/spec/06_storage_engine.md` now documents the
  reclamation model (collapse + policy hook + the "tombstones retained
  pending PR2" note) and corrects the stale "(user key ASC, sequence DESC)"
  merge-order claim; `docs/spec/18_concurrency.md` notes that compaction
  reclaims space for overwrites.
- The cross-device "no resurrection" scenario is tracked from
  [plan_harness_mixed_storage.md](../plan_harness_mixed_storage.md) Step 5
  plus a release-checklist entry that PR2 will register, rather than as a
  standalone plan.
- D1 was revised during sign-off (recorded in the Decisions section above):
  the original "drop-immediately when local-only" recommendation was
  retracted as unsafe under retroactive local → synced transitions; the
  confirmed rule is always-require-a-horizon, with a wall-clock
  `tombstoneGraceDuration` for local-only databases. The horizon
  computation lands in PR2.
- `make pre_commit` clean; 1402 tests pass (+17 vs `main` at PR open).
