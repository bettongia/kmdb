...
| `CKError.quotaExceeded` shape                                                                | Not empirically triggered. Handled defensively per Apple docs: the plugin maps this to a `QUOTA_EXCEEDED` error, which crosses the method channel as a `PlatformException` (there is no `KmdbException` type in the workspace; only rate-limiting has a bespoke type, `ICloudRateLimitException`).                                                                                                                                     |
