...
| Android  | Supported — `.so` resolved from Maven AAR |
| iOS      | **Not yet supported** — ORT ships a static `ar archive`; Flutter enforces dynamic link mode. SPM plugin shim required (tracked in `betto_onnxrt`). `OnnxRuntime.load()` throws `UnsupportedError` on iOS until the shim is implemented. |
| Web      | Excluded — semantic search is not supported on web (§20) |

### Workspace wiring

`betto_onnxrt` follows the same workspace wiring pattern as `betto_zstd`:
