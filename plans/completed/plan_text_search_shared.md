...
- The `Tokenizer` interface and `RegExpTokenizer` implementation, currently
...
- `Tokenizer` interface and `RegExpTokenizer` are pure Dart and belong in `kmdb`
...
- `IcuTokenizer` (ICU C FFI) goes in `kmdb_tokenizer_icu` to keep FFI out of
  `kmdb`. This mirrors how `kmdb_zstd` holds Zstd FFI. The implementation can be
  moved directly from `spikes/icu_tokenizer/` with minor adjustments.
...
- `search()` on `KmdbCollection<T>` is stubbed to throw `UnimplementedError` in
  this plan. Plans 2 and 3 replace the stub with real implementations.

### Key files to create / modify

| Action | Path                                                      |
| :----- | :-------------------------------------------------------- |
| Create | `packages/kmdb/lib/src/search/tokenizer.dart`             |
| Create | `packages/kmdb/lib/src/search/regexp_tokenizer.dart`      |
...
- `RegExpTokenizer` on an empty string must return an empty list without error.
- `IcuTokenizer` construction failure (library not found) must throw
...
- [x] Scaffold `packages/kmdb_tokenizer_icu/` using
      `dart create --template=package`:
  - `pubspec.yaml`: `publish_to: none`, `resolution: workspace`, depends on
    `kmdb` and `ffi: ^2.x`
  - `analysis_options.yaml`: include workspace root file
- [x] Scaffold `packages/kmdb_inferencing/` using
      `dart create --template=package`:
  - `pubspec.yaml`: `publish_to: none`, `resolution: workspace`, depends on
    `kmdb`, `ffi: ^2.x`, `path: ^1.x`
  - `analysis_options.yaml`: include workspace root file
...
- [x] Create `packages/kmdb/lib/src/search/fts_index_definition.dart`:
  - `class FtsIndexDefinition` with `collection`, `field`, `lazy` (default
    false)
...
- [x] Create `packages/kmdb/lib/src/search/vec_index_definition.dart`:
  - `class VecIndexDefinition` with `collection`, `field`, `lazy` (default
    false)
- [x] Export all new types from `packages/kmdb/lib/kmdb.dart`
- [x] Tests (`packages/kmdb/test/search/`):
  - [x] `regexp_tokenizer_test.dart`: empty string, single word, prose sentence,
...
- [x] Move `IcuTokenizer` from `spikes/icu_tokenizer/lib/src/icu_tokenizer.dart`
      into `packages/kmdb_tokenizer_icu/lib/src/icu_tokenizer.dart`
  - Update `import` path for `Tokenizer` to reference `package:kmdb/kmdb.dart`
  - Update license header
- [x] Create `packages/kmdb_tokenizer_icu/lib/kmdb_tokenizer_icu.dart` barrel
      exporting `IcuTokenizer`
- [x] Move tests from `spikes/icu_tokenizer/test/` into
      `packages/kmdb_tokenizer_icu/test/icu_tokenizer_test.dart`
  - All shared `Tokenizer` contract tests
...
- [x] Add validation: if `vecIndexes.isNotEmpty && embeddingModel == null`,
      throw
      `ArgumentError('embeddingModel is required when vecIndexes is non-empty')`
- [x] Store the parameters on the `KmdbDatabase` instance for use by
      `FtsManager` and `VecManager` in later plans; stub both managers as null
      for now
...
- [x] Add `Future<SearchResult<T>> search(String query, {...})` to
      `KmdbCollection<T>` in `packages/kmdb/lib/src/query/kmdb_collection.dart`:
  ```dart
...
- [x] Create directory structure:
  ...
  ```
- [x] Add `.gitattributes` entry for `*.onnx` files →
...
- Created two new workspace packages: `kmdb_tokenizer_icu` (ICU FFI tokenizer)
  and `kmdb_inferencing` (ONNX Runtime embedding model scaffold), both joined to
  the workspace and resolving correctly.
- Added 7 shared types to `kmdb/lib/src/search/`: `Tokenizer`,
  `RegExpTokenizer`, `SearchMode`, `SearchResult<T>`, `SearchMetadata`,
  `SearchHit<T>`, `EmbeddingModel`, `FtsIndexDefinition`, `VecIndexDefinition` —
  all exported from `kmdb.dart`.
- Moved `IcuTokenizer` from the spike into `kmdb_tokenizer_icu` with updated
...
- 69 new tests added across 3 new test files; zero analyzer warnings; all
  existing tests continue to pass (pre-existing Zstd native asset failures are
