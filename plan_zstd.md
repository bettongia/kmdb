...
an x86_64 macOS blob (`eszstd-mac64.dylib`). On ARM64 machines the blob must
be compiled from source using the package's blob-builder tool:
...
Until then, ARM64 developers must run the blob-builder once after `dart pub get`.
...
- `lib/src/encoding/compression_web.dart` — Deflate encode+decode (unchanged logic)
...
- `hook/build.dart` — removed (no longer needed; `es_compression` bundles binaries)
- `test/encoding/value_codec_test.dart` — Zstd round-trip, cross-flag, threshold tests
